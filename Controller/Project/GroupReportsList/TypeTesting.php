...
class TypeTesting extends Base
{
    public function execute($context)
    {
        ...
        $availableGenerators = array();                
        if ($this->registry->getModel('Auth')->checkAccess($_SESSION['accesses'], 'project_edit', $context['project']['projectID'])) {
            $availableGenerators = $generators; // админам доступно все
        }
        else {
            $userGenerators = $this->registry->getDbHelper('UsersHelper')->getProjectUserGenerators($context['project']['projectID'], $context['USER']->GetID());
            foreach ($userGenerators as $userGenerator) {
                $availableGenerators[$userGenerator['report_generatorID']] = $generators[$userGenerator['report_generatorID']];
            }
        }
        
        $this->component->arResult['generators'] = $availableGenerators;
        ...
        $reports = $this->registry->getDbHelper('ReportsHelper')->getGroupReportsByProjectIDAndUserID($context['project']['projectID'],
            $context['USER']->GetID(), $page, $onPage, 'DESC');
        ...
        if (isset($_POST['generator']) && array_key_exists($_POST['generator'], $availableGenerators)) {
        
            $valid = false;
            $params = array();
            if ($availableGenerators[$_POST['generator']]['use_valuation_group']) { // добавляем нормогруппу в настройки
                if (isset($_POST['valuation_group']) && $_POST['valuation_group']) {
            ...
            if ($valid) {
                ...
                $taskID = $this->registry->getDbHelper('TasksHelper')->addTask(array(
                    ...
                    'params'  => json_encode(array_merge($params, array('projectID' => $context['project']['projectID'], 'generatorID' => $_POST['generator']))),
