    ...
    public function execute($context)
    {
        ...
        if ($this->component->arParams['ACTION_MODE'] == 'add') {
            ...
            if ($projectAdmin['access_type'] == UsersHelper::USER_ACCESS_TYPE_FLAGS) {  // получаем установленные группы
                ...
                $userGenerators = $this->registry->getDbHelper('UsersHelper')->getProjectUserGenerators($context['project']['projectID'], $userID);
                foreach ($userGenerators as $generator) {
                    $this->component->arResult['form_data']['generator_'.$generator['report_generatorID']] = 1;
                }
            }
        }
        ...
        if ($this->component->arParams['ACTION_MODE'] == 'add' && !count($this->component->arResult['availableUsers'])) { // нет незадействованных пользователей
            ...
            // генераторы групповых отчетов
            $generators = $this->registry->getDbHelper('ReportsHelper')->getGeneratorsByProjectTypeAndReportTypes('testing', array('GROUP'));
                ...
                elseif ($_POST['rights_type'] == UsersHelper::USER_ACCESS_TYPE_FLAGS && count($_POST) <= 3) { // TODO: поумней способ проверки
                    ...
                    if ($this->component->arParams['ACTION_MODE'] == 'add') {
                        ...
                        if ($_POST['rights_type'] == UsersHelper::USER_ACCESS_TYPE_FULL
                            ...
                            $this->registry->getDbHelper('UsersHelper')->deleteProjectUserGenerators($context['project']['projectID'], $projectAdmin['userID']);
                            if ($userGenerators && count($userGenerators)) {
                                foreach ($userGenerators as $generator) {
                                    $this->registry->getModel('ActionsLogger')->log($context['USER']->GetID(), 'project_user_generator', $generator['report_generatorID'],
                                        LogsHelper::ACTION_TYPE_DELETE, $generator);
                                }
                            }
                        }
                        elseif ($_POST['rights_type'] == UsersHelper::USER_ACCESS_TYPE_FLAGS) { // флаги всегда обновляем                            
                            ...
                            // очищаем старые ссылки на генераторы
                            $this->registry->getDbHelper('UsersHelper')->deleteProjectUserGenerators($context['project']['projectID'], $projectAdmin['userID']);
                            if ($userGenerators && count($userGenerators)) {
                                foreach ($userGenerators as $generator) {
                                    $this->registry->getModel('ActionsLogger')->log($context['USER']->GetID(), 'project_user_generator', $generator['report_generatorID'],
                            ...
                            if ($userData['at_group_reports']) {
                                foreach ($_POST as $key => $value) { // добавляем новые
                                    if (strpos($key, 'generator_') === 0) {
                                        $generatorID = intval(substr($key, 10));
                                        $projectUserGeneratorID = $this->registry->getDbHelper('UsersHelper')->addProjectUserGenerator(array(
                                            'projectID' => $context['project']['projectID'],
                                            'userID'  => $projectAdmin['userID'],
                                            'report_generatorID'  => $generatorID,
                                        ));
                                        $this->registry->getModel('ActionsLogger')->log($context['USER']->GetID(), 'project_user_generator',
                                            $projectUserGeneratorID, LogsHelper::ACTION_TYPE_ADD, null);
                                    }
                                }
                            }
                        }
